...
def get_1sec_frames(data, features, algo):
    ...
    while point < rows:
        ...
        processed_df.loc[point] = list(map(lambda x: results[x] if x != 'autocorrelation_2' else results['rms'], features))
...
def main(output_path, features, clf):
    ...
    try:
        a = autocorr(data['autocorrelation_2'], 'full')
        data['autocorrelation_2'] = pd.DataFrame(a)
    except KeyError:
        print('No autocorrelation_2')
